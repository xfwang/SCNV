...
##The functions below can produce CBS results based on the coverage matrix.

#gn.norm : function to perform GC normalziation
#CBS :  wrapper function to conduct CBS segmentation
#plot.baslan:  Plot the segmentation as in Baslan 2012
#cne : A function to estiamte copy number and plot the desnity of segment value differences. modified from the Baslan code
...
gc.norm<-function (badbins=NA, gc.data, coverage) {
	...
	thisRatio<-coverage
	if (nrow(gc)!= nrow(thisRatio)) {stop("files not matched")}
...
plot.baslan<-function (thisRatio, sample.id="cell") {
	...
	#png(paste(sample.id, ".wg.png", sep=""))
...
cne<- function (df,dfs) {
	...
	a3 <- amat[(1:counter),1]
	a3.99 <- quantile(a3,0.99)
	a3.new<-a3[which(a3 < a3.99)]
	a3d<-density(a3.new,bw=.02) #<-bw is important here, suggest 0.01~0.05
